...
case class ReportModel(request: ReportRequest) {
  ...
  val d = managed(new ByteArrayOutputStream()) map { os =>
    ...
    for (item <- request.details) {
      ...
      // table.addValue(new CellParams(new LocalDate(item.getWhen. ) item.getValue().getWhat(), col3width, HorizontalAlignment.LEFT))
