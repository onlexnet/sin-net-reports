...
- If needed, prompt the user if they want to integrate static analysis tools (PMD, Checkstyle)
...
- **Null Handling**: Avoid returning or accepting `null`. Use JSpecify annotations for possibly-absent values and `Objects` utility methods like `equals()` and `requireNonNull()`.
