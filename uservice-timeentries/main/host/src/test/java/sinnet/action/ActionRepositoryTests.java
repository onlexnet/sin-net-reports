...
public class ActionRepositoryTests {
  ...
  public void myTest() {
    ...
    var customerId = api.defineCustomer(ShardedId.anyNew(projectEid));
    ...
    Sync.of(() -> projection.find(projectId, now, now))
      ...
      Sync.of(() -> sutWrite.save(ShardedId.anyNew(projectId), newEntity))
          .and(it -> sutWrite.save(ShardedId.anyNew(projectId), newEntity))
          .and(it -> sutWrite.save(ShardedId.anyNew(projectId), newEntity))
  ...
  public void shouldSaveFullModel() {
    ...
    Sync
        .of(() -> sutWrite.save(ShardedId.anyNew(projectId), newEntity))
