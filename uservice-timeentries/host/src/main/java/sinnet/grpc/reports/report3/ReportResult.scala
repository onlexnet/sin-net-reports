...
object ReportResult {
  ...
  object TableColumns {
    def Col1widthDescription = new TableColumn(3)
    def Col2widthNoted = new TableColumn(7)
    def width = Col1widthDescription.width +
  ...
  private def asTable(addValue: (Boolean, CellParams*) => Unit, activities: Seq[GroupDetails]): Unit = {

    for (item <- activities) {
      val personName = item.personName
      val clients = item.details
